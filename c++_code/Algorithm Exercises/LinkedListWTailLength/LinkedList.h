...
LinkedList<T>::~LinkedList() {
    ...
    if(current == nullptr){goto endofdestruct;}
    else{
...
LinkedList<T>::LinkedList(const LinkedList<T>& other) {
    ...
    length = 0;
    for(int stepsLeft = other.length; stepsLeft > 0; stepsLeft--) {
        insertEnd(current->ListNode<T>::data);
...
LinkedList<T>& LinkedList<T>::operator=(const LinkedList<T>& other) {
    if(this != &other) {
        ...
        ListNode<T>* current = other.head;
        for(int stepsLeft = length; stepsLeft > 0; stepsLeft--){
            removeAt(0);
        }
        for(int stepsLeft = other.length; stepsLeft > 0; stepsLeft--){
